...
def has_option(name):
    x = GetOption(name)
    return x != False and x is not None and x != ''
...
def setup_quiet_build(env, colorblind=False):
    ...
    compile_cxx_msg = f"{c['blue']}[CXX]{c['rst']} {c['yellow']}$SOURCE{c['rst']}"

    compile_c_msg = f"{c['cyan']}[CC]{c['rst']} {c['yellow']}$SOURCE{c['rst']}"

    compile_shared_msg = (
        f"{c['bold_blue']}[SHR]{c['rst']} {c['yellow']}$SOURCE{c['rst']}"
    )

    link_program_msg = f"{c['bold_magenta']}[LNK exe]{c['rst']} {c['bold'] + c['yellow'] + c['inverse']}$TARGET{c['rst']}"

    link_lib_msg = f"[LIB st]{c['rst']} {c['cyan']}$TARGET{c['rst']}"

    ranlib_library_msg = f"[RANLIB]{c['rst']} {c['cyan']}$TARGET{c['rst']}"

    link_shared_library_msg = (
        f"{c['bold_magenta']}[LNK shr]{c['rst']} {c['bold']}$TARGET{c['rst']}"
    )

    pch_compile = f"{c['bold_magenta']}[PCH]{c['rst']} {c['bold']}$SOURCE{c['rst']} -> {c['bold']}$TARGET{c['rst']}"
